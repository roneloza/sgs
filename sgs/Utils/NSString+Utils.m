...
+ (NSString *)stringUrlPathWithHost:(NSString *)host stringPaths:(NSArray *)paths {
    
    NSString *pathApiUser = [[NSString alloc] initWithFormat:@"%@", host];
    
    for (__weak NSString *path in paths) {
        
        pathApiUser = [pathApiUser stringByAppendingPathComponent:[[NSString alloc] initWithFormat:@"%@", path]];
    }
